...
// //     // MODIFIED LOGIC: Determine the row and column based on the current user's perspective
// //     if (isCurrentUserPlayer1) {
// //       // P1's View: Pits 0-6 are bottom, Pits 8-14 are top (reversed)
// //       isTopRowPit = pitIndex > 7;
// //       col = isTopRowPit ? 14 - pitIndex : pitIndex;
// //     } else {
// //       // P2's View: Pits 8-14 are bottom, Pits 0-6 are top (reversed)
// //       if (pitIndex >= 8 && pitIndex <= 14) {
// //         // P2's pits (bottom row, 8-14)
// //         isTopRowPit = false;
// //         col = pitIndex - 8;
// //       } else {
// //         // P1's pits (top row, 0-6)
// //         isTopRowPit = true;
// //         col = 6 - pitIndex;
// //       }
// //     }
...
// //         if (!isBotTurn && isPlayer1Turn && nextIndex == 15) {
// //           nextIndex = (nextIndex + 1) % animatingBoard.length;
// //         } else if ((!isPlayer1Turn || isBotTurn) && nextIndex == 7) {
// //           nextIndex = (nextIndex + 1) % animatingBoard.length;
// //         }
...
// //       if (result['isEnded']) {
// //         final finalBoard = result['finalBoard'];
// //         final outcome = GameLogic.getWinnerPlayer(finalBoard);
// //         await _handleGameOver(
// //           outcome,
// //           finalBoard,
// //           player1Id,
...
// //     barrierLabel: 'Resign Game',
// //     transitionDuration: const Duration(milliseconds: 300),
// //     pageBuilder: (context, anim1, anim2) {
// //       return const SizedBox.shrink(); // Content handled in transitionBuilder
// //     },
...
// //                 // Deep, sophisticated dark gradient for a premium feel
// //                 gradient: const LinearGradient(
...
// //                   const Text(
// //                     "Resigning forfeits the match. Are you sure you want to surrender?",
// //                     style: TextStyle(
...
// //                 _DialogButton(
// //                 text: 'Back to Home',
// //                 color: Colors.transparent, // Dark Red
// //                 onPressed: onGoHome,
// //               ),
// //             ],
// //           ),
...
//     // <CHANGE> Game over is now shown as a dialog overlay instead of replacing the screen
//     if (winner != null || isGameOver) {
//       Future.microtask(() {
//         _showGameOverDialog(
//           data,
...
//   // <CHANGE> New game over dialog using showGeneralDialog for overlay effect (same as resignDialog)
...
//       message = "Congratulations! You Won!";
//       localWinnerId = currentUser!.uid;
//       resultColor = const Color(0xFFFACC15);
...
//                     mainAxisSize: MainAxisSize.min,
//                     children: [
//                       Icon(icon, size: 48, color: resultColor),
...
String? _pendingRematchId;
  ...
  bool isMakingMove = false;
  bool botThinking = false;
  // <CHANGE> Add flag to prevent duplicate game over dialogs
  ...
  void _animateBoardChange(List<int> prevBoard, List<int> newBoard) {
    ...
    if (sourcePit == -1) return;

    // Simulate the pebble distribution animation
    _animatePebbleDistribution(prevBoard, newBoard, sourcePit);
  }

  // <CHANGE> Animate pebble distribution based on board changes
  Future<void> _animatePebbleDistribution(
    List<int> prevBoard,
    List<int> newBoard,
    int sourcePit,
  ) async {
    List<int> animatingBoard = List.from(prevBoard);
    int stones = prevBoard[sourcePit];
    animatingBoard[sourcePit] = 0;
    int index = sourcePit;
    ...
    for (int i = 0; i < stones; i++) {
      ...
      final isPlayer1 = currentUser!.uid == matchData!['player1Id'];
      if (isPlayer1 && nextIndex == 15) {
        nextIndex = (nextIndex + 1) % animatingBoard.length;
      } else if (!isPlayer1 && nextIndex == 7) {
        nextIndex = (nextIndex + 1) % animatingBoard.length;
      }
  ...
  Widget build(BuildContext context) {
    ...
    // <CHANGE> Only show game over dialog once using flag
    if ((winner != null || isGameOver) && !_gameOverDialogShown) {
