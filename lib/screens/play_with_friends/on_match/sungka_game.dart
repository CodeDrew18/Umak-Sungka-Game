...
//     titleText.render(canvas, 'MANCALA', Vector2(screenWidth / 2 - 80, 20));
...
//   int lastPitIndex = -2; // -2 means store, >=0 means pit
...
//     if (isPlayer1Turn) {
//       // Player 1 distributes left → right
//       if (idx < pitsPerSide) {
//         currentPits[idx]++;
...
//       }
//     } else {
//       // Player 2 distributes right → left
//       if (idx < pitsPerSide) {
...
//   // Free turn: last stone in player's store
//   if (lastPitIndex == -1) {
//     turnAgain = true;
//   }
...
class SungkaGame extends FlameGame with TapDetector, HasGameRef {
  ...
  String? winner;
  bool isAnimating = false;
  bool showInfo = false; // Added flag to show game info

  late TextPaint pitText;
  late TextPaint storeText;
  late TextPaint titleText;
  late TextPaint turnText;
  late TextPaint infoButtonText; // Added text paint for info button
  ...
  void render(Canvas canvas) {
    ...
    if (showInfo) {
      _drawInfoOverlay(canvas, screenWidth, screenHeight);
    }
  ...
  void _drawInfoButton(Canvas canvas, double screenWidth) {
    ...
    infoButtonText.render(
      canvas,
      'INFO',
      Vector2(buttonX + 8, buttonY + 8),
    );
  }
  ...
  void _drawInfoOverlay(Canvas canvas, double screenWidth, double screenHeight) {
    ...
    //   canvas,
    //   GameInfo.objective,
    //   Vector2(boxLeft + 20, boxTop + 50),
    // );

    // contentPaint.render(
    //   canvas,
    //   GameInfo.setup,
    //   Vector2(boxLeft + 20, boxTop + 100),
    // );

    // contentPaint.render(
    //   canvas,
    //   GameInfo.freeTurn,
    //   Vector2(boxLeft + 20, boxTop + 150),
    // );

    final tipsPaint = TextPaint(
      style: const TextStyle(
        color: Color(0xFF8B4513),
        fontSize: 14,
        fontWeight: FontWeight.bold,
    ...
    tipsPaint.render(
      canvas,
      'Game Tips:',
      Vector2(boxLeft + 20, boxTop + 200),
    );

    contentPaint.render(
      canvas,
      '• Plan moves ahead like chess',
      Vector2(boxLeft + 20, boxTop + 225),
    );

    contentPaint.render(
      canvas,
      '• Land in your store for free turns',
      Vector2(boxLeft + 20, boxTop + 245),
    );

    contentPaint.render(
      canvas,
      '• Use capture rule strategically',
      Vector2(boxLeft + 20, boxTop + 265),
    );

    final closePaint = TextPaint(
      style: const TextStyle(
        color: Color(0xFF666666),
        fontSize: 14,
        fontFamily: 'Arial',
    ...
    closePaint.render(
      canvas,
      'Tap to close',
      Vector2(boxLeft + 180, boxTop + 550),
    );
  }
  ...
  void onTapDown(TapDownInfo info) {
    ...
    if (pos.x > screenWidth - 80 && pos.x < screenWidth - 20 && pos.y > 20 && pos.y < 60) {
      showInfo = !showInfo;
      return;
    }
