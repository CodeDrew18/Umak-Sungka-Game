...
//     final String message = 'Final Score';
...
//     // --- Original Navigation Logic Remains ---
//     if (exitGame == true) {
//       Navigator.push(
//         context,
//         MaterialPageRoute(
...
//     }
//   },
//   // The IconButton itself is also styled for a better look
//   style: IconButton.styleFrom(
...
//     for (int i = 0; i < maxPebbles; i++) {
//       final dx =
//           _random.nextDouble() * (widget.size * 0.5) + (widget.size * 0.1);
//       final dy =
//           _random.nextDouble() * (widget.size * 0.5) + (widget.size * 0.1);
//       final double pebbleSize = 6.0;
//       final rotation = _random.nextDouble() * pi;
...
//     //     isPlayerTwoStore ? const Color(0xFF8B5CF6) : const Color(0xFFFF4D67);
...
final pitSize = screenWidth < 600 ? 48.0 : 66.0;
    final boardWidth = screenWidth > 900 ? 900.0 : screenWidth;
    final boardInnerWidth = boardWidth - 24.0; // Subtract padding/border (12 + 12)
const _boardTexture = DecorationImage(
      image: AssetImage(_woodenTexturePath),
      fit: BoxFit.cover,
    );
...
if (winner == 'player1') {
    ...
    return Scaffold(
      backgroundColor: Colors.transparent,
      body: SingleChildScrollView( // Added for vertical overflow
        child: Column(
          children: [
            Container(
              padding: const EdgeInsets.all(12),
...
return PitWidget(
 ...
 pitSize,
                                          boardInnerWidth, // Use inner width
                                        ),
    ...
    onPressed: () async {
      bool?
exitGame = await showGeneralDialog<bool>(
        context: context,
        barrierDismissible: false,
        barrierLabel: 'Exit Game',
        transitionDuration: const Duration(milliseconds: 300),
...
const Color(0xFFFACC15) : Colors.redAccent;
    final icon = isWin ? Icons.emoji_events : Icons.close_sharp;
return Center(
      // Ensure the dialog content is scrollable if needed
      child: SingleChildScrollView(
